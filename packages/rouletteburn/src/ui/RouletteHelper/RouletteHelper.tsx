...
function RouletteHelper() {
	const [gameState, setGameState]: [GameState, (gameState: GameState) => void] =
		React.useState({
			chipLocation: -1,
			lastClick: -1,
	...
	return (
		<>
			...
			<p>
				Recent gambles:{" "}
				{gameState.chipLocation >= 0
					? gameState.playthrough
							.getRecentGuesses(numberOfResultsToDisplay)
							.concat([gameState.chipLocation.toString().padStart(2, "0")])
							.join(", ")
					: gameState.playthrough
							.getRecentGuesses(numberOfResultsToDisplay)
							.join(", ")}
			...
			<p>
				Recent results:{" "}
				{gameState.playthrough
					.getRecentResults(numberOfResultsToDisplay)
					.join(", ")}
			</p>
			<table className={css.table}>
				<tr>
					...
					<RouletteCell
						num={3}
						className={`${css.cell} ${css.topmost}`}
						gameState={gameState}
				...
				<tr>
					<RouletteCell
						num={0}
						className={`${css.cell} ${css.leftmost}`}
						gameState={gameState}
						updateGameState={updateGameState}
					/>
					<RouletteCell
