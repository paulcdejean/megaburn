...
export class RoulettePlaythrough {
	...
	public addRound(round: RouletteRound) {
		...
		if (this.playthroughStartTime < 0) {
			this.playthroughStartTime = Date.now();
		}
		...
		if (this.rounds.length === this.seedCalculateRound) {
			...
			if (this.seed >= 0) {
				...
				for (
					let spin = 0;
					spin < this.predictedResult.spins.length + this.predictedResult.skips;
					spin++
				) {
	...
	private getInitialSeed() {
		const possibleResults = 2 ** this.rounds.length;
	...
	public getRecentResults(count: number): string[] {
		return this.rounds
			.slice(Math.max(0, this.rounds.length - count))
			.map((round) => round.result.toString().padStart(2, "0"));
	}

	public getRecentGuesses(count: number): string[] {
		return this.rounds
			.slice(Math.max(0, this.rounds.length - count))
			.map((round) => round.guess.toString().padStart(2, "0"));
	}
}
